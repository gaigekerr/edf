...
def harmonize_afacs(vintage, statefips):
    ...
    for tract in np.arange(0, len(tracts), 1):
        ...
        if np.isnan(pm_inside)!=True:
            ...
            # Lower RR
            afpmihdlower, afpmstlower = np.nan, np.nan
            afpmcopdlower, afpmlclower = np.nan, np.nan
            afpmdmlower, afpmlrilower = np.nan, np.nan
            afpmihdwho15lower, afpmstwho15lower = np.nan, np.nan
            afpmcopdwho15lower, afpmlcwho15lower = np.nan, np.nan
            afpmdmwho15lower, afpmlriwho15lower = np.nan, np.nan
            afpmihdnaaqs12lower, afpmstnaaqs12lower = np.nan, np.nan
            afpmcopdnaaqs12lower, afpmlcnaaqs12lower = np.nan, np.nan
            afpmdmnaaqs12lower, afpmlrinaaqs12lower = np.nan, np.nan                
            ...
            # Upper RR
            afpmihdupper, afpmstupper = np.nan, np.nan
            afpmcopdupper, afpmlcupper = np.nan, np.nan
            afpmdmupper, afpmlriupper = np.nan, np.nan
            afpmihdwho15upper, afpmstwho15upper = np.nan, np.nan
            afpmcopdwho15upper, afpmlcwho15upper = np.nan, np.nan
            afpmdmwho15upper, afpmlriwho15upper = np.nan, np.nan
            afpmihdnaaqs12upper, afpmstnaaqs12upper = np.nan, np.nan
            afpmcopdnaaqs12upper, afpmlcnaaqs12upper = np.nan, np.nan
            afpmdmnaaqs12upper, afpmlrinaaqs12upper = np.nan, np.nan                
